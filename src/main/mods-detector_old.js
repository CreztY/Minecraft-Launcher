...
/**
 * Guarda información de mods verificados en archivo de configuración
 * para detectar cambios manuales en futuras comprobaciones
 */
export function saveModsChecksum(modsDir, modsList) {
  try {
    const checksumFile = join(modsDir, '.mods-checksum.json')
    const checksumData = {}

    modsList.forEach((mod) => {
      checksumData[mod.filename] = {
        name: mod.name,
        version: mod.version,
        hash: mod.installedHash,
        size: mod.fileSize,
        checkedAt: new Date().toISOString()
      }
    })

    writeFileSync(checksumFile, JSON.stringify(checksumData, null, 2))
    console.log('Mods checksum saved to:', checksumFile)
  } catch (error) {
    console.error('Error saving mods checksum:', error)
  }
}

/**
 * Verifica si los mods han sido modificados comparando con checksum guardado
 */
export function checkForModsModified(modsDir, installedMods) {
  try {
    const checksumFile = join(modsDir, '.mods-checksum.json')
    ...
    const previousChecksum = JSON.parse(readFileSync(checksumFile, 'utf-8'))
    const modified = []

    installedMods.forEach((mod) => {
      const previous = previousChecksum[mod.filename]
      if (previous && previous.hash !== mod.installedHash) {
        modified.push({
          ...mod,
        ...
        console.log(`Mod modified: ${mod.name} (hash changed)`)
      }
    })

    return modified
  } catch (error) {
    console.error('Error checking for modified mods:', error)
    return []
  }
}
